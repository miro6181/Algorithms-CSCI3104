...
\item \label{2b} 
	...
	\end{verbatim}
	\end{small}
\makenonemptybox{2.5in}{ret stores either $-1$ or i at the start of the i\textsuperscript{th} iteration. Where i is the index of the found number.}
\pagebreak
...
\item \label{2c}
	...
	\end{verbatim}
	\end{small}
\makenonemptybox{2.5in}{sum is storing A[0]+A[1]+A[2]+ ... + A[i] at the start of the i\textsuperscript{th} iteration.}
\end{enumerate}
...
\pagebreak
 ...
 %	\item {\itshape (1 pt)  Prove by induction that for each $n \in \mathbb{Z}^{+}$, $(2n)! < 2^{2n} (n!)^{2}$.}
	\makenonemptybox{3in}{Base Case - $i = 0$ : $\sum_{i=0}^{0} r^{0} = 1 = \frac{(1-r^{0+1})}{1-r} = 1$ and thus proves our base case.
